...
export default class ConversionToolbar extends Module<ConversionToolbarNodes> {
  ...
  public async replaceWithBlock(replacingToolName: string): Promise<void> {
    ...
    const currentBlockClass = currentBlock.class;
  ...
  public async mergeBlocks(replacingToolName: string): Promise<void> {
    ...
    blocksData.forEach((data, index) => {
      ...
      const currentBlockClass = currentBlock.class;
      const exportProp = currentBlockClass[INTERNAL_SETTINGS.CONVERSION_CONFIG] ? currentBlockClass[INTERNAL_SETTINGS.CONVERSION_CONFIG].export : null;
      ...
      if (_.isFunction(exportProp)) {
        ...
        _.log(`Conversion «export» property not defined on ${currentBlock.class.toString()}`);
      }
    });
    ...
    let newBlockData = {};
    const conversionConfig = replacingTool[INTERNAL_SETTINGS.CONVERSION_CONFIG];
